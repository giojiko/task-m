...
export function generatePassportUrlId() {
  if (typeof crypto !== 'undefined' && crypto.randomUUID) {
    return crypto.randomUUID();
  }
...
// Readable 12-char temp password — letters + digits, no ambiguous chars (0/O, 1/l/I)
...
export async function initDB() {
  ...
  const adminUser = {
    id: uid(), name: 'Giorgi Jikia', firstName: 'Giorgi', lastName: 'Jikia',
    email: 'giorgi.j@smartpro.ge',
  ...
  const db = {
    ...
    directions: [
      ...
      { id: 'd10', name: 'გაყიდვა', nameEn: 'Sales', nameRu: 'Продажа', desc: 'პროდუქციის გაყიდვა მომსახურების გარეშე', color: '#F59E0B', icon: '🛒' },
    ],
  };
  return { db, tempPassword, adminUser };
}
