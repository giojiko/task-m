...
export async function sendPasswordResetEmail(employee, newPassword) {
  const html = wrap(`
    <h2 style="color:#F0F4F8;font-size:20px;font-weight:700;margin-bottom:4px">
      🔐 პაროლი განახლდა
    </h2>
    <p style="color:#8B9BB4;font-size:14px;margin-bottom:24px">
      ${employee.firstName} ${employee.lastName}, თქვენი SmartPro-ს ანგარიშის პაროლი ადმინისტრატორმა განაახლა.
    </p>
    <div style="background:#0D1117;border-radius:10px;padding:16px 20px;margin-bottom:20px;border:1px solid #1E2537">
      <div style="color:#8B9BB4;font-size:11px;text-transform:uppercase;letter-spacing:.05em;margin-bottom:4px">
        ახალი პაროლი
      </div>
      <div style="color:#1BEACD;font-size:22px;font-weight:800;font-family:monospace;letter-spacing:.08em">
        ${newPassword}
      </div>
    </div>
    <p style="color:#8B9BB4;font-size:12px;margin-bottom:20px">
      ⚠️ გთხოვთ პირველივე შესვლისას შეიცვალოთ პაროლი.
    </p>
    <a href="https://office.smartpro.ge/login"
      style="display:inline-block;background:linear-gradient(135deg,#1BEACD,#13B89E);
      color:#0D1117;padding:12px 28px;border-radius:8px;font-weight:700;
  ...
  return getResend().emails.send({
    from: FROM,
    to: employee.email,
    subject: '🔐 თქვენი SmartPro პაროლი განახლდა',
    html,
  });
}
